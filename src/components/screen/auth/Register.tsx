...
export function Register() {
	const { register, handleSubmit, formState: { errors }, reset } = useForm<IAuthRegister>({
		mode: 'onChange'
	})
	const { register: authRegister, isRegister } = useRegister()
	...
	return <div className='flex-center flex-col h-full'>
		<Image 
			src='/icon.svg' 
			alt='Logo' 
			width={50} 
		...
		>
			<Input 
				...
				className={cn('w-85 py-2 px-3 border outline-none rounded-xl mt-10 transition-color', {
					['border-gray-400 focus:border-white hover:border-white']: !errors['name'],
					['border-red-700 focus:border-red-700']: !!errors['name']
				})}
			/>
			<Input 
			...
			<button 
				className={cn('btn-primary w-85 px-3 py-2 mt-5 hover:animate-popUp transition-transform cursor-pointer', {
					['opacity-50 cursor-not-allowed']: isRegister
				})}
				disabled={isRegister}
			>
				Sign up</button>
